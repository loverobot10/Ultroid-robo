import asyncio
import random
from userbot import bot as R2K_VENOM
...
@R2K_VENOM.on(admin_cmd(incoming=True))
@R2K_VENOM.on(sudo_cmd(incoming=True, allow_sudo=True))
async def _(event):
    global que
    ...
    async with event.client.action(event.chat_id, "typing"):
        await event.client.send_message(
            ...
            message="""{}""".format(random.choice(VENOM)),
            reply_to=event.message.id,
        )

@R2K_VENOM.on(admin_cmd(pattern="replyraid(?: |$)(.*)"))
@R2K_VENOM.on(sudo_cmd(pattern="replyraid(?: |$)(.*)", allow_sudo=True))
async def _(event):
    global que
    if event.fwd_from:
        return
...
@R2K_VENOM.on(admin_cmd(pattern="dreplyraid(?: |$)(.*)"))
@R2K_VENOM.on(sudo_cmd(pattern="dreplyraid(?: |$)(.*)", allow_sudo=True))
async def _(event):
    global que
    if event.fwd_from:
        return
